...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	// My methods
	public ListaGenerica<T> invert() {
		ListaGenerica<T> mList = new ListaGenericaEnlazada<>();
		this.comenzar();
		for (int i=0; i<this.tamanio; i++){
			mList.agregarInicio(proximo());
		}
		return mList;
	}

	public ListaGenerica<T> invertRec(){
		if (!this.fin()) {
			T actual = this.proximo();
			this.agregarInicio(actual);
