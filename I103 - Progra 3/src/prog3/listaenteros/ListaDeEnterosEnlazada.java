...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	// My Methods
	public int queTantoLesCostabaImplementarEsto(){
		return actual.getDato();
	}


	// Precondition 1: all of the integers > 0
	// Precondition 2: all of the integers are different
	public ListaDeEnteros sort(){
		ListaDeEnteros result = new ListaDeEnterosEnlazada();
		int actual, min, last = 0;
		...
		for (int i=0; i<this.tamanio(); i++){
			...
			while (!this.fin()){
				actual = this.proximo();
				if (actual < min && actual > last)
					min = actual;
			}

			result.agregarFinal(min);
			last = min;
		}
	...
	// Precondition 1: paramList is already sorted
	// Precondition 2: instance list is already sorted too
	public ListaDeEnteros combineSort(ListaDeEnteros paramList){
		if (this.esVacia()) return paramList;
		if (paramList == null || paramList.esVacia()) return this;
		...
		int a = this.proximo();
		int b = paramList.proximo();

		for (int i=0; i<paramList.tamanio()+this.tamanio(); i++) {
			if (a < b){
				result.agregarFinal(a);
				if (this.fin()) a = Integer.MAX_VALUE;
				else a = this.proximo();
			} else {
				result.agregarFinal(b);
				if (paramList.fin()) b = Integer.MAX_VALUE;
				else b = paramList.proximo();
			}
		}
