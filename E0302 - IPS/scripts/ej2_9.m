...
%% Caracteristicas
h = impz(FIR_BS);       % Respuesta impulsional
h = [h' zeros(1,2048)];
n = [0:1:length(h)-1];
...
title('Módulo de la respuesta en frecuencia del filtro rechaza-banda');
xlabel('f [Hz]'); ylabel('|H(f)| [dB]');

%% Gráfica de la fase de la respuesta en frecuencia
figure; plot(f,unwrap(angle(H)) + 570.31);
title('Fase de la respuesta en frecuencia del filtro rechaza-banda');
